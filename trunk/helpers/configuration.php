<?php
/**
 * @version		$Id: configuration.php 12681 2009-09-08 13:42:04Z hackwar $
...
require_once(JPATH_ADMINISTRATOR .DS.'components'.DS.'com_jupgrade'.DS.'helpers'.DS.'database.php' );
...
class JInstallationModelConfiguration extends JModel
{
	...
	function _createConfiguration($options)
	{
		...
		$registry->setValue('root_user', 62);
		...
		$registry->setValue('host', $options->host);
		$registry->setValue('user', $options->user);
		$registry->setValue('password', $options->password);
		$registry->setValue('db', $options->db);
